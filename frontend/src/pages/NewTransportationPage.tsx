...
interface Values {
  ...
  originPoint?: string; destinationPoint?: string; loadingAddress?: string; loadingDateTime?: Dayjs; loadingContactName?: string;
  ...
  subcontractorRateCurrency?: string; vatMode?: string; plannedStartDate?: Dayjs; vehicleNumber?: string; trailerNumber?: string;
...
const dateTime = (value?: Dayjs) => value?.toISOString();
...
export function NewTransportationPage() {
  ...
  const submit = async () => {
    ...
    const initialLeg = compact({
      ...
      vatMode: raw.vatMode, plannedStartDate: date(raw.plannedStartDate), vehicleNumber: raw.vehicleNumber, trailerNumber: raw.trailerNumber,
    ...
    const payload = compact({ ...raw, plannedDeliveryDate: date(raw.plannedDeliveryDate), loadingDateTime: dateTime(raw.loadingDateTime),
      plannedStartDate: undefined, driverLicenseDate: undefined, initialLeg });
    ['subcontractorId','subcontractorRate','subcontractorRateCurrency','vatMode','vehicleNumber','trailerNumber','driverFullName','driverPhone','driverIin','driverLicenseNumber','driverLicenseIssuer'].forEach((key) => delete (payload as Record<string, unknown>)[key]);
    setSaving(true);
  ...
  return <section className="transportation-workspace wizard-page">
    ...
    <Card className="transport-card wizard-card">
      <Form form={form} layout="vertical" initialValues={{ transportMode: 'AUTO', placesUnit: 'паллеты', cargoValueCurrency: 'USD', clientRateCurrency: 'USD', subcontractorRateCurrency: 'KZT', vatMode: 'NO_VAT', isDangerous: false, isTempControlled: false }}>
        ...
        <div style={{ display: step === 2 ? undefined : 'none' }}><Typography.Title level={4}>{t('transportationWizard.sections.route')}</Typography.Title><div className="form-grid two">
          ...
          <Collapse className="optional-collapse" items={[{ key: 'details', label: t('transportationWizard.sections.addresses'), children: <div className="form-grid two">{field('loadingAddress', t('transportationWizard.fields.loadingAddress'))}{field('loadingDateTime', t('transportationWizard.fields.loadingDateTime'), <DatePicker showTime className="full-width" />)}{field('loadingContactName', t('transportationWizard.fields.loadingContact'))}{field('loadingContactPhone', t('transportationWizard.fields.loadingPhone'))}{field('unloadingAddress', t('transportationWizard.fields.unloadingAddress'))}{field('unloadingContactName', t('transportationWizard.fields.unloadingContact'))}{field('unloadingContactPhone', t('transportationWizard.fields.unloadingPhone'))}<Form.Item name="bodyType" label={t('transportationWizard.fields.bodyType')}><Select options={['TENT','REFRIGERATOR','CONTAINER','OPEN_PLATFORM','ISOTHERM'].map((value) => ({ value: t(`transportationWizard.bodyTypes.${value}`), label: t(`transportationWizard.bodyTypes.${value}`) }))} /></Form.Item>{field('specialConditions', t('transportationWizard.fields.specialConditions'), <Input.TextArea />)}</div> }]} />
          ...
          <Card size="small" title={t('transportationWizard.sections.carrier')}><div className="form-grid two"><Form.Item name="subcontractorId" label={t('transportationWizard.fields.carrier')}><Select showSearch filterOption={false} onFocus={() => void findCarriers()} onSearch={(value) => void findCarriers(value)} options={carriers.map((item) => ({ value: item.id, label: item.name }))} /></Form.Item><Form.Item label={t('transportationWizard.fields.subcontractorRate')}>{money('subcontractorRate','subcontractorRateCurrency')}</Form.Item><Form.Item name="vatMode" label={t('transportationWizard.fields.vat')}><Select options={['NO_VAT','VAT_12'].map((value) => ({ value, label: t(`transportationWizard.vat.${value}`) }))} /></Form.Item>{field('plannedStartDate', t('transportationWizard.fields.plannedPickup'), <DatePicker className="full-width" />)}{field('vehicleNumber', t('transportationWizard.fields.vehicle'))}{field('trailerNumber', t('transportationWizard.fields.trailer'))}</div>
