...
interface Values {
  ...
  originPoint?: string; destinationPoint?: string; isDomestic?: boolean; loadingAddress?: string; loadingContactName?: string;
...
export function NewTransportationPage() {
  ...
  const summary = useMemo(() => [
    ...
    [t('transportationWizard.routeType'), values.isDomestic === true ? t('transportationWizard.routeTypes.domestic') : values.isDomestic === false ? t('transportationWizard.routeTypes.international') : undefined],
    ...
    <Card className="transport-card wizard-card">
      <Form form={form} layout="vertical" initialValues={{ transportMode: 'AUTO', placesUnit: 'паллеты', cargoValueCurrency: 'USD', clientRateCurrency: 'KZT', subcontractorRateCurrency: 'KZT', isDangerous: false, isTempControlled: false }}>
        ...
        <div style={{ display: step === 2 ? undefined : 'none' }}><Typography.Title level={4}>{t('transportationWizard.sections.route')}</Typography.Title><div className="form-grid two">
          <Form.Item name="originPoint" label={t('transportationWizard.fields.origin')} rules={[{ required: true, message: t('transportationWizard.validation.origin') }]}><Input /></Form.Item><Form.Item name="destinationPoint" label={t('transportationWizard.fields.destination')} rules={[{ required: true, message: t('transportationWizard.validation.destination') }]}><Input /></Form.Item><Form.Item name="isDomestic" label={t('transportationWizard.routeType')} className="span-all"><Radio.Group optionType="button" buttonStyle="solid" options={[{ value: true, label: t('transportationWizard.routeTypes.domestic') }, { value: false, label: t('transportationWizard.routeTypes.international') }]} /></Form.Item></div>
          ...
          {canSeeClientRate && <Card size="small" title={t('transportationWizard.sections.client')}><Form.Item label={t('transportationWizard.fields.clientRate')}>{money('clientRate','clientRateCurrency','KZT')}</Form.Item></Card>}
