...
Ты — **исполнитель кода** в проекте CRM AVA Solution. Оркестратор (Claude Code) ставит тебе задачи через MCP-мост Codex (инструменты `codex` / `codex-reply`) либо, если мост не подключён, через `tools\codex-task.cmd`; он же проводит ревью, тестирует и коммитит.
